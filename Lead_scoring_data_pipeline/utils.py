...
def build_dbs():
    ...
    if os.path.isfile(DB_PATH+DB_FILE_NAME):
        ...
        try:
            ...
            print(e)
            return "Error occured while creating new database"
        finally:
            if conn:
                conn.close()
...
def load_data_into_db():
    ...
    print('Reading the leadscoring_inference.csv file...')
    ...
    df_lead_scoring.to_sql(name='loaded_data', con=conn_db, if_exists='replace', index=False)
...
def map_city_tier():
    ...
    df_lead_scoring = pd.read_sql('select * from loaded_data', conn_db)
    ...
    df_lead_scoring.to_sql(name='city_tier_mapped', con=conn_db, if_exists='replace', index=False)
    print('city_tier mapped')
...
def map_categorical_vars():
    ...
    df_lead_scoring = pd.read_sql('select * from city_tier_mapped', conn_db)
    ...
    df.to_sql(name='categorical_variables_mapped', con=conn_db, if_exists='replace', index=False)
    print('categorial variables mapped')
...
def interactions_mapping():
    ...
    df = pd.read_sql('select * from categorical_variables_mapped', conn_db)
    ...
    # df_pivot.to_sql(name='interactions_mapped', con=conn_db, if_exists='replace', index=False)
    
    #df_model_input = df_pivot.drop(NOT_FEATURES, axis=1)
    
    df_pivot.to_sql(name='model_input', con=conn_db, if_exists='replace', index=False)
    print('final model input table created')
    
    # Save the data into the csv file
    df_pivot.to_csv('/home/airflow/dags/Lead_scoring_data_pipeline/data/cleaned_data.csv', index=False)
